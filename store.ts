...
export function loadStore(){try{store={...empty(),...JSON.parse(fs.readFileSync(storePath,"utf8")) as RunStore};for(const negotiation of Object.values(store.negotiations)){negotiation.calls??=[];negotiation.callIds??=[]}}catch{store=empty()}return store}
