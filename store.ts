...
export function loadStore(){try{store={...empty(),...JSON.parse(fs.readFileSync(storePath,"utf8")) as RunStore};for(const negotiation of Object.values(store.negotiations)){negotiation.mode??="SANDBOX";negotiation.calls??=[];negotiation.callIds??=[];negotiation.providers??=[];negotiation.evidence??=[];negotiation.verifiedFacts??=[];negotiation.policyDecisions??=[];for(const call of negotiation.calls){call.phase??="QUOTE_COLLECTION";call.twilioCallSid??=null;call.draft??=null;if(call.conversationId==="pending")call.conversationId=null}}}catch{store=empty()}return store}
