...
export function recommend(negotiationId: string) {
  return mutate(() => { const n = get(negotiationId); const latest = [...n.offers].sort((a,b) => b.offerVersion - a.offerVersion)[0]; if (!latest?.totals.statedAllInMinor) throw new Error("at least one priced offer is required"); const total = latest.totals.statedAllInMinor; let action: ActionRecommendation["action"] = "COUNTER"; const reasons: string[] = []; if (latest.comparability !== "COMPARABLE" || latest.totals.reconciliation !== "MATCH") { action = "CLARIFY"; reasons.push("The offer is not yet a reconciled, comparable all-in quote."); } else if (n.redFlags.some(r => r.severity === "HIGH")) { action = "CLARIFY"; reasons.push("Resolve high-severity risks before deciding."); } else if (total > n.strategy.walkAwayMinor) { action = "WALK_AWAY"; reasons.push("The offer exceeds the user's walk-away point."); } else if (total <= n.strategy.realisticTargetMinor) { action = "ACCEPT"; reasons.push("The offer meets the approved realistic target."); } else { reasons.push("The offer is inside the acceptable range but above target."); }
