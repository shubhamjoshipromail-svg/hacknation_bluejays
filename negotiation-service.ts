...
function refreshBenchmark(n:NegotiationType){const comparable=n.offers.filter(o=>o.stage==="INITIAL"&&o.comparability==="COMPARABLE"&&o.totals.reconciliation==="MATCH"&&o.totals.statedAllInMinor!=null);const byProvider=new Map(comparable.map(o=>[o.providerId,o.totals.statedAllInMinor!]));if(byProvider.size<2)return;const totals=[...byProvider.values()].sort((a,b)=>a-b);const middle=Math.floor(totals.length/2);n.benchmark={lowMinor:totals[0],typicalMinor:totals.length%2?totals[middle]:Math.round((totals[middle-1]+totals[middle])/2),highMinor:totals.at(-1)!,classification:"VERIFIED",sourceLabel:`${totals.length} reconciled local phone quotes for the confirmed VIN specification`,notes:["This benchmark is derived from comparable itemized quotes gathered for this negotiation.","Negotiated revisions are evaluated separately so they do not distort the initial market range."],generatedAt:now()}}
