...
export function buildServer(env:NodeJS.ProcessEnv=process.env,deps:{startCall?:CallStarter}={}){const app=Fastify({logger:false,bodyLimit:1_000_000});app.addContentTypeParser("application/json",{parseAs:"buffer"},(_req,body,done)=>done(null,body));app.addHook("onSend",async(_req,reply,payload)=>{reply.header("access-control-allow-origin",(_req.headers.origin as string|undefined)??env.FRONTEND_ORIGIN??"*");reply.header("access-control-allow-headers","content-type,x-tool-secret,ngrok-skip-browser-warning");reply.header("access-control-allow-methods","GET,POST,OPTIONS");return payload});app.options("*",async(_req,reply)=>reply.code(204).send());
  ...
  app.post<{Params:{id:string}}>("/api/negotiations/:id/reservation-call",async req=>{const b=z.object({providerId:z.string().min(1)}).parse(jsonBody(req.body));const n=getNegotiation(req.params.id);if(!n.approvals.some(a=>a.action==="ACCEPT_OFFER"))throw new Error("explicit ACCEPT_OFFER approval is required before a reservation call");if(!n.providers.some(p=>p.providerId===b.providerId))throw new Error("provider not found");return (deps.startCall??startElevenLabsCall)(req.params.id,b.providerId,"CONFIRMATION")});
